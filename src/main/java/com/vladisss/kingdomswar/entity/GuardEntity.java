...
public class GuardEntity extends PathfinderMob {
        ...
        if (!level().isClientSide && guardPost != null) {
            ...
            if (distanceFromPost > MAX_DISTANCE_FROM_POST * MAX_DISTANCE_FROM_POST) {
                ...
                if (target != null && target.isAlive()) {
                    ...
                    double targetDistance = target.distanceToSqr(guardPost.getX(), guardPost.getY(), guardPost.getZ());
    ...
    protected void registerGoals() {
        ...
        this.targetSelector.addGoal(2, new net.minecraft.world.entity.ai.goal.target.NearestAttackableTargetGoal<>(
                ...
                false,
                (monster) -> {
                    // Атаковать монстров только если они близко к замку
                    if (guardPost == null) return false;
                    double distanceToPost = monster.distanceToSqr(guardPost.getX(), guardPost.getY(), guardPost.getZ());
                    return distanceToPost < 20 * 20; // Только в радиусе 20 блоков от поста
                }
        ));
